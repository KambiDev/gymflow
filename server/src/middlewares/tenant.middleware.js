...
export async function requireActiveTenant(req, res, next) {
  ...
  if (req.user.role === 'super_admin' || !req.user.tenantId) {
    return res.status(403).json({
      error: 'El super admin no tiene acceso a los datos operativos de los gimnasios.'
    });
  }
  ...
  try {
    ...
    if (statusInfo.status === 'suspended') {
      return res.status(403).json({
        ...
        reason: tenant.suspended_reason || 'Por favor contacta al administrador de la plataforma para reactivar tu cuenta.'
      });
    }
